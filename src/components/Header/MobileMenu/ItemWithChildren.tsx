import { Box, AccordionItem, AccordionButton, AccordionPanel, AccordionIcon, Heading, Text } from '@chakra-ui/react';
...
const CustomMenuItem = ({children, ...other}: CustomMenuProps) => (
    <Text {...other} fontWeight={500} fontSize='13px' lineHeight={'19.5px'} _hover={{backgroundColor: "rgba(44,19,56,.04)", borderRadius: "8px" }} 
...
const ChildMenuIcon = ({iconName}: {iconName: string}) => {
    if(iconName === "DocumentationIcon") { return (<DocumentationIcon />)}
    if(iconName === "APIReferenceIcon") { return (<APIReferenceIcon />)}
    if(iconName === "CommunityIcon") { return (<CommunityIcon />)}
    if(iconName === "StatusIcon") { return (<StatusIcon />)}
    if(iconName === "BlogIcon") { return (<BlogIcon />)}
    if(iconName === "HelpCenterIcon") { return (<HelpCenterIcon />)}
    return <p />
}
...
const ItemWithChildren = ({title, items, key}: ItemWithChildrenProps) => (
    <AccordionItem key={title} p="28px 0 20px">
        ...
        <AccordionPanel pb={0} pt={6}>
            {
                items.map(childMenu => (
                    <MetaLink to={childMenu.link} linkType={childMenu.isExternalLink ? "external": "internal" }>
