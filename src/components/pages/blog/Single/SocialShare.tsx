...
const SocialShare = ({ slug, title }: { slug: string; title: string }) => {
  ...
  return (
    <Flex mb={{ base: 5, md: 0 }}>
      <ExternalLink
        to={`http://twitter.com/share?text=${title}&url=${blogUrl}`}
      >
        <TwitterShareIcon />
      </ExternalLink>
      <ExternalLink
        to={`https://www.linkedin.com/sharing/share-offsite/?url=${blogUrl}`}
      >
        <Box px={4}>
          <LinkedInShareIcon />
        </Box>
      ...
      <ExternalLink
        to={`http://www.reddit.com/submit?url=${blogUrl}&title=${title}`}
      >
        <RedditShareIcon />
      </ExternalLink>
    </Flex>
