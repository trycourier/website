...
type PlanNameType = "developerData" | "starterData" | "growthData" | "enterpriseData";
...
const RightText = ({children, planNameKey}: RightTextProps) => {
    if(children === "nodata") {
        return (
            <Box bg="rgba(44,19,56,.16)" width="10px" height="2px" borderRadius="10px" verticalAlign="center" />
        )
        ...
        return (
            <Text variant="body3" background="linear-gradient(270deg,#9121c2,#ff5e5e)" sx={{
        ...
        return (
            <Text variant="body3medium">{children}</Text>
        )
    }
        ...
        <Box color="secondary.dark">
            <PlanCTA planNameKey={planNameKey} />
            {
                data.map((section, sectionIndex) => (
                    <Box key={section.title} pb={section.title == "Support & services" ? "22px": 0}>
                        <Heading mb={"16px"} variant="subh1" mt={sectionIndex === 0 ? "24px": "32px"}>{section.title}</Heading>
                        {
                            section.features.map((feature) => (
                                <Flex key={feature.label} w={'100%'} p="8px 0px" justify="space-between">
                                    <Flex align="center" flexWrap="wrap">
                                        <Text variant="body3" color="rgba(0,0,0,.72)">{feature.label}</Text>
                                        {
                                            feature.tag &&
                                            <Tag sx={{
