...
const FreeTrialButton = ({buttonType, children, height, width, link, ...other}: FreeTrialButtonProps) => {
    if(buttonType === "WhiteBgSimple") {
        ...
        return (
            <DarkGradientBg height={height} width={width} link={link} {...other}>{children}</DarkGradientBg>
        )
    } else {
        return (
            <PurpleBgButton height={height} width={width} link={link} {...other}>{children}</PurpleBgButton>
        )
    }
}
